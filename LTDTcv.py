...
train = spark.read.csv('hdfs://tmp/DA-12-v3.csv', inferSchema=True,header=True)
...
dtc = DecisionTreeClassifier(labelCol='EventCode_indexer',featuresCol='features')
...
pipeline = Pipeline(stages=[Actor1_indexer, Actor2Code_indexer, Actor1Country_indexer, Actor2Country_indexer,
                            ...
                            Actor2Country_encoder,EventCode_indexer, Region_encoder, assembler, dtc])



paramGrid = ParamGridBuilder()\
  .addGrid(dtc.maxDepth, [2, 4, 6])\
  .build()
...
pp_Acc.saveAsTextFile("hdfs:///tmp/Result")
result = prediction.select("EventCode_indexer", "prediction", "probability")
result.show()
print('A Decision Tree algorithm had an accuracy of: {0:2.2f}%'.format(pp_Acc*100))
